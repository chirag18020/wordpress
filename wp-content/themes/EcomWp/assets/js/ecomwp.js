 ...
 /*----------------------------------------*/
	...
	jQuery(document).ready(function($) {
	 $('.uren-add_cart').on('click', function(e) { 
		...
		//id = thisbutton.val();
		let id = thisbutton.data('product_id');
		...
		$(document.body).trigger('single_add_to_cart_button', [thisbutton, data]);
		 $.ajax({
            type: 'post',
            ...
            success: function (response) {
                if (response.error && response.product_url) {
					...
					console.log('yesssss111');
                    return;
                } else {
					console.log('yesssss');
                    $(document.body).trigger('single_add_to_cart_button', [response.fragments, response.cart_hash, thisbutton]);
	...
	//*****ajax rageslider filter search product*****//
	/* jQuery(document).ready(function($) {
		jQuery('.ui-slider-handle, #slider-range').on('click', function(e){
			e.preventDefault();
			
		});
	}); */
	
	//*****ajax sidebar filter search product*****//
	...
	jQuery(document).ready(function($) {
		$('.module-list_item li a').on('click', function(e) {
			e.preventDefault();
			...
			$(function () {
				...
				amountprice.val('$' + sliderrange.slider('values', 0) + ' - $' + sliderrange.slider('values', 1));
				alert(JSON.stringify(amountprice));
			}); */
			 
			
			var slidecost = jQuery('.price-slider-amount #amount').val();
			var costarray = slidecost.split(" - ");
			 var minval = JSON.stringify(costarray[0]);
			 var maxval = JSON.stringify(costarray[1]);
			 jQuery('#minprice').val(minval);
			 jQuery('#maxprice').val(maxval);

			$.ajax({
				url: ecomwpAjax.ajaxurl,
				type: 'POST',
				data: {
